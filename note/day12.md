...
用mutex替换读写锁。
读写锁的优势在于读是不阻塞的，但是代价较大。用mutex可以保证读写时的数据一致性；用shared_ptr判断写的时候是否有人在读。如果有人在读，那么就新建一个对象，拷贝Map，将现有指针指向新建对象，在新建对象上修改。这样是为了防止在原有数据上修改导致读取的数据不一样。在修改前执行的查询会查询到旧数据，之后的会查询到新数据。
```cpp
...
class CustomerData:boost::noncopyable
{
    ...
    static int findEntry(const EntryList& entries_,const strings& stock);
...
{
    MapPtr data=getData();//getData用锁保护，其余部分没有锁保护
    //对于读来说自然保护不保护没啥关系，对于写则不一样
...
{
    MutexLockGuard lock(mutex_);//加锁，没有进程正在getData
    if(!data.unique())//有进程正在读
    {
        MapPtr newData(new Map(*data_));//复制Map给新对象
        data_.swap(newData);//新旧对象互换，现在CustomerData指向新对象，正在读的那些进程指向旧对象
    }
    assert(data_.unique());//没有进程在读，直接修改
    (*data_)[customer]=entries;//修改
    //mutex保证了在修改时没人在getdata
    //shared_ptr保证了修改时没人在查看对象，要是有就另外新建一个对象
}
```
...
如果我们不想f函数改变参数的值，可以把函数原型改为f(constint),但如果我们不允许f改变this指向的对象呢？因为this是隐含参数，const没法直接修饰它，就加在函数的后面了，表示this的类型是constA *const this。
